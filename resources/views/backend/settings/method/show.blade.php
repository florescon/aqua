...
<div class="row">
  <div class="col-md-12 mb-4">
    <ul class="nav nav-tabs" role="tablist">
      ...
      <li class="nav-item">
        <a class="nav-link @if (Request::query('page') == 'inscriptions') active @endif" data-toggle="tab" href="#inscriptions" role="tab" aria-controls="profile">
          <i class="icon-basket-loaded"></i> @lang('labels.backend.access.cash_out.table.inscriptions')
          @if($inscriptions->count())
              <span class="badge badge-pill badge-success">{{ $inscriptions->total() }}</span>
          @endif        
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link @if (Request::query('page') == 'payments') active @endif" data-toggle="tab" href="#payments" role="tab" aria-controls="messages">
          <i class="icon-pie-chart"></i> @lang('labels.backend.access.cash_out.table.monthly_payments')
          @if($payments->count())
              <span class="badge badge-pill badge-success">{{ $payments->total() }}</span>
          @endif
        </a>
      </li>
      @if($method->id == 2)
    ...
    <div class="tab-content">
        ...
        <table class="table table-responsive-sm">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                @if(!empty($sale->box))
                  <a href="{{ route('admin.transaction.cash.show', $sale->box) }}">#{{ $sale->box }}</a>
                @else
                  <span class="badge badge-pill badge-secondary"> <em>Sin definir</em></span>
                @endif
              </td>
              <td>
                @php($total=0)
      ...
      <div class="tab-pane @if (Request::query('page') == 'inscriptions') active @endif" id="inscriptions" role="tabpanel">
        <table class="table table-responsive-sm">
          <thead>
            <tr>
              <th>Folio</th>
          ...
          <tbody>
            ...
            <tr>
              <td>
                <a href="{{ route('admin.subscription.subscription.show', $inscription->id) }}">#{{ $inscription->id }}</a>
              </td>
              ...
              <td>
                @if(!empty($inscription->box))
                  <a href="{{ route('admin.transaction.cash.show', $inscription->box) }}">#{{ $inscription->box }}</a>
                @else
                  <span class="badge badge-pill badge-secondary"> <em>Sin definir</em></span>
                @endif
              </td>
        ...
        <div class="row">
            <div class="col-7">
                <div class="float-left">
                    {!! $inscriptions->total() !!} {{ trans_choice('labels.backend.access.subscription.table.total', $inscriptions->total()) }} - <button class="btn btn-brand btn-sm btn-dropbox">{{ $method->name }}</button>
                </div>
            </div><!--col-->

            <div class="col-5">
                <div class="float-right">
                    {{ $inscriptions->appends(['page' => 'inscriptions'])->links() }}
                </div>
            </div><!--col-->
        </div><!--row-->
      </div>
      <div class="tab-pane @if (Request::query('page') == 'payments') active @endif" id="payments" role="tabpanel">
        <table class="table table-responsive-sm">
          <thead>
            <tr>
              <th>Folio</th>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                @if(!empty($payment->box))
                  <a href="{{ route('admin.transaction.cash.show', $payment->box) }}">#{{ $payment->box }}</a>
                @else
                  <span class="badge badge-pill badge-secondary"> <em>Sin definir</em></span>
                @endif
              </td>
        ...
        <div class="row">
            <div class="col-7">
                <div class="float-left">
                    {!! $payments->total() !!} {{ trans_choice('labels.backend.access.payment.table.total', $payments->total()) }} - <button class="btn btn-brand btn-sm btn-dropbox">{{ $method->name }}</button>
                </div>
            </div><!--col-->

            <div class="col-5">
                <div class="float-right">
                    {{ $payments->appends(['page' => 'payments'])->links() }}
                </div>
            </div><!--col-->
        </div><!--row-->
      </div>
      ...
      <div class="row">
        ...
        <div class="col-sm-6 col-lg-3">
          <div class="card text-white bg-success">
            <div class="card-body">
                <i class="nav-icon fa fa-credit-card float-right"></i>
              <div class="text-value">${{ number_format($method->total_inscription, 2, ".", ",") }}</div>
        ...
        <div class="col-sm-6 col-lg-3">
          <div class="card text-white bg-success">
            <div class="card-body">
                <i class="nav-icon fa fa-clipboard-list float-right"></i>
              <div class="text-value">${{ number_format($method->total_payment, 2, ".", ",") }}</div>
              <div>Mensualidades</div>
            </div>
          </div>
        </div>
        <!-- /.col-->
      ...
      <div class="row">
        ...
        @php($totalcard=0)
        @php($totalcard+=$method->total_sales+$method->total_inscription+$method->total_payment+$cardsincome-$cardsexpense)
        <div class="col-sm-6 col-lg-3">
