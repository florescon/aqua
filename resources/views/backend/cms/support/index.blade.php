...
@section('title', app_name() . ' | ' . __('labels.backend.access.cms_support.management'))
        ...
        <div class="row">
            <div class="col-sm-5">
                <h4 class="card-title mb-0">
                    {{ __('labels.backend.access.cms_support.management') }}
                </h4>
            </div><!--col-->
            ...
            <div class="col">
                <div class="table-responsive">
                    <table class="table">
                        <thead>
                        <tr>
                            <th>@lang('labels.backend.access.cms_support.table.title')</th>
                            <th>@lang('labels.backend.access.cms_support.table.content')</th>
                            <th>@lang('labels.backend.access.cms_support.table.meta_title')</th>
                            <th>@lang('labels.backend.access.cms_support.table.meta_description')</th>
                            <th>@lang('labels.backend.access.cms_support.table.meta_keywords')</th>
                        ...
                        @foreach($pages as $page)
                            <tr>
                                ...
                                <div class="btn-group" role="group" aria-label="_{{ _('labels.backend.access.users.user_actions') }}">
                                    ...
                                    <a href="{{ route('admin.cms.support.edit', $page->id) }}" data-placement="top" title="{{ __('buttons.general.crud.edit') }}" class="btn btn-primary"><i class="fas fa-edit"></i></a>

    								<a href="{{ route('admin.cms.support.destroy', $page->id) }}" class="btn btn-danger" data-method="delete" data-trans-button-cancel="{{ __('buttons.general.cancel') }}" data-trans-button-confirm="{{ __('buttons.general.crud.delete') }}" data-trans-title="{{ __('strings.backend.general.are_you_sure') }}" class="dropdown-item">
        ...
        <div class="row">
            <div class="col-7">
                <div class="float-left">
                    {!! $pages->total() !!} {{ trans_choice('labels.backend.access.cms_support.table.total', $pages->total()) }}
                </div>
            </div><!--col-->
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="createPageLabel">@lang('labels.backend.access.cms_support.create')</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>

    <form autocomplete="off" method="POST" action="{{ route('admin.cms.support.store') }}">
       @csrf
