...
class TagController extends Controller
{
    ...
    {
    	// $sections = Tag::orderBy('updated_at', 'desc')->where('type', 1)->paginate();
    	// return view('backend.settings.tag.index', compact('sections'));

        if ($request->ajax()) {
            $data = Tag::query()->where('type', 1);
        ...
        return view('backend.settings.tag.index');
	...
	return redirect()->route('admin.class.tag.index')
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('admin.class.tag.index')->withFlashSuccess('Etiqueta eliminada con éxito');
    }

    public function select2LoadMore(Request $request)
    {
        $search = $request->get('search');
        $data = Tag::select(['id', 'name'])->where('name', 'like', '%' . $search . '%')->orderBy('name')->where('type', 1)->paginate(5);
